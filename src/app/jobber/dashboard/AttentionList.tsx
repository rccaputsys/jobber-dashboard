...
/**
 * Reusable "Here's what needs your attention" panel. Shows the top N
 * actions by default with a "Show more" button to reveal the rest.
 * If an action has an `href`, the row renders as a clickable link.
 */
export function AttentionList({
  ...
  return (
    <div data-tour={tourId || "attention-list"} className="panel animate-in delay-1" style={{ padding: "14px 18px", marginBottom: 12 }}>
        ...
        <>
          <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
            {visible.map((action, i) => {
              ...
              return action.href ? (
                <a key={i} href={action.href} className="hover-lift" style={rowStyle(action.color)}>
                  {inner}
                </a>
              ) : (
                <div key={i} style={rowStyle(action.color)}>
                  {inner}
                </div>
              );
            })}
